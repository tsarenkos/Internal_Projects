using System.Diagnostics;
using Microsoft.AspNetCore.Mvc;
using Microsoft.Extensions.Logging;
using Autofac_App.Models;
using Autofac_App.Services;

namespace Autofac_App.Controllers
{
    public class HomeController : Controller
    {
        private readonly ILogger<HomeController> _logger;

        private readonly IDateTimeWithOffsetService _dateTimeWithOffsetService;

        public HomeController(ILogger<HomeController> logger, IDateTimeWithOffsetService dateTimeWithOffsetService)
        {
            _logger = logger;

            _dateTimeWithOffsetService = dateTimeWithOffsetService;
        }

        [Route("time")]
        public IActionResult GetTime(double offset)
        {
            return Content(_dateTimeWithOffsetService.GetDateTime(offset).ToString());
        }

        public IActionResult Index()
        {
            return View();
        }

        public IActionResult Privacy()
        {
            return View();
        }

        [ResponseCache(Duration = 0, Location = ResponseCacheLocation.None, NoStore = true)]
        public IActionResult Error()
        {
            return View(new ErrorViewModel { RequestId = Activity.Current?.Id ?? HttpContext.TraceIdentifier });
        }
    }
}
